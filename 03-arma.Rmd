...
```{r, fig.height= 4.5, fig.width= 5, fig.align='center', fig.cap="Estimated PACF of a time series from an AR(3) model with parameters $\\phi_1 = 1.2$, $\\phi_2 = -0.9$ and $\phi_3 = 0.3$."}
set.seed(1)
...
```{r, fig.height= 5.5, fig.width= 7.5, fig.align='center', fig.cap="Plot of the AIC, BIC and HQ model selection criteria from all models included in an AR(10) model. The observed time series was simulated from an AR(3) model.", echo = FALSE}
...
For the purpose of the simulation we consider a sample size $T = 100$ and $B = 500$ bootstrap replications. The code used for the simulation study (being lengthy) is omitted for practical exposition purposes.
```
