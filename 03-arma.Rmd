...
```{r diagnostic_plot, fig.align='center', fig.height=6, fig.width=9.5, fig.cap="TO DO."}
set.seed(333)
...
lynx_gts = gts(log(lynx), start = 1821, data_name = "Lynx Trappings", unit_time = "year", name_ts = "Trappings (log)")
plot(lynx_gts)
```
...
```{r, fig.asp = 0.6, fig.width = 7, fig.align='center', fig.cap="TO DO", echo = FALSE}
...
We consider sample size $N = 100$ and bootstrap replications $B = 1000$. The code for this simulation is long and is therefore omitted. 
```
...
```{r, cache = TRUE, fig.asp = 0.6, fig.width = 7, fig.align='center', fig.cap="TO DO"}
plot(auto_corr(lynx_gts, pacf = TRUE)) 
```

As we saw earlier, the PACF appears to be significant at lag $h = 11$ therefore leading to the conclusion that an AR(11) model would be good to describe/forecast the observed time series. However, let us check if we reach the same conclusion using the three model selection criteria described above. For this reason, let us use an AR(16) as the maximum order for the candidate AR($p$) models.

```{r, eval = FALSE}
lynx_select = select(AR(16), lynx_gts, include.mean = TRUE)
```

```{r, eval = TRUE, cache = TRUE, echo = FALSE, fig.asp = 0.6, fig.width = 7, fig.align='center', fig.cap="TO DO"}
...
As we can see, an AR(11) model is selected which is equivalent to the model selected by the AIC and HQ criteria. Moverover, it can observed that MAPE is quite variable and that several models have MAPE close to the one of the selected model. As the MAPE is prone to overfitting a reasonable approach is to select a model based on the "one standard deviation rule" which in this case would corespond to an AR(3), the same as the model selected by the BIC. Therefore also this selection method would appear to indicate that either an AR(11) or an AR(3) appears to be "good" models for this time series. 
