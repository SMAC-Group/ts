...
```{r, fig.height= 5.5, fig.width= 7.5, fig.align='center', fig.cap="TO DO.", eval = FALSE}
best_model = select(AR(10), Xt, include.mean = FALSE) 
```


```{r, fig.height= 5.5, fig.width= 7.5, fig.align='center', fig.cap="TO DO.", echo = FALSE}
...
The first one corresponds to a "strong" signal while the second one is called a "weak" signal. The third one is an example of an AR process of infinte dimension. Indeed, we have

Based on the theoretical (i.e. asymptotic) properties of model selection criteria, we expect the BIC to perform very well for the first model, while the AIC should give "good" results on the second model.
...
B = 500
result1 = result2 = result3 = data.frame(aic = rep(NA, B), 
                               bic = rep(NA, B), 
                               hq = rep(NA, B))
...
for(i in 1:B){
  ...
  Xt3 = gen_gts(n = 100, MA(theta = 0.9, sigma2 = 1))
  ...
  tmp_aic = select(AR(9), Xt3, criterion = "aic", plot = FALSE)
  tmp_bic = select(AR(9), Xt3, criterion = "bic", plot = FALSE)
  tmp_hq = select(AR(9), Xt3, criterion = "hq", plot = FALSE)
  
  result3$aic[i] = length(tmp_aic$model$phi)
  result3$bic[i] = length(tmp_bic$model$phi)
  result3$hq[i] = length(tmp_hq$model$phi)
}
...
aic1 = bic1 = hq1 = aic2 = bic2 = hq3 = aic3 = bic3 = hq3 = rep(0, 9)
...
aic3[as.numeric(names(counts2_aic))] = as.vector(counts3_aic)
bic3[as.numeric(names(counts2_bic))] = as.vector(counts3_bic)
hq3[as.numeric(names(counts2_hq))] = as.vector(counts3_hq)
...
```{r, fig.align='center', fig.height=8, fig.width=10, fig.cap="TO DO", echo = FALSE, warning=FALSE, message=FALSE, error=FALSE}
# plot the results
par(mfrow=c(2,2))
mycolor = hcl(h = seq(15, 375, length = 4), l = 65, c = 100, alpha = 1)
