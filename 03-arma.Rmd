...
where $\mathbf{H} \equiv \mathbf{X} \left(\mathbf{X}^T \mathbf{X}\right)^{-1} \mathbf{X}$ denotes the ``Hat'' matrix (as it puts a `r emo::ji("tophat")` on $\mathbf{Y}$). Then,
the theoretical $C$ becomes:
...
Alternatively, several researchers used in the 1970's a different approach based on the Kullback-Leibler disprepancy, which is ofen called the *Information Theory* approach. While a theoretical discussion of this method is clearly beyond the scope of this document, the general idea is essentially the same as Mallow's $C_p$, where one would constructed a theoretical quantity (such as $C$ in our previous example) and compute/derive a suitable estimator of this quantity. The most famous information theory-based model selection crierion is the landmark Akaike Information Criterion or AIC [see @akaike1974new], which is probably the most commonly used model selection crierion for time series data. In the late 1970's there was an explosion of work in this field and many model selection criteria were proposed including the Bayesian Information Criterion or BIC proposed by @schwarz1978estimating and the HQ criterion proposed by @hannan1979determination.  

MAYBE ADAPT A BIT THIS PARAGRAPH BELOW:
Assuming that this is a good criterion to evaluate the "goodness" of a model, most of the quantites defining this criterion are not directly available and therefore we need estimators for this criterion. Denoting $\text{logL}(\hat{\boldsymbol{\theta}})$ as being the log-likelihood function evaluated at the estimated parameters and $k$ as the number of parameters in the model, the most common criteria that aim at estimating this quantity are defined as follows:
...
From a theoretical standpoint it is difficult to whether the AIC, BIC or HQ (or many other criteria) is better. Essentially, statisticiens have considered to general philosophie of model selection. The first one is known as *model selection efficiency* and is largely due to @shibata1980asymptotically. This paradigm relies on the common assumption (in both time series and regression) that the data generating process (or true model) is of **infinite** dimension and/or that the sets of candidate models (i.e. the set of models we are considering). Under this setting to goal of model selection is to find the candidate that is the ``closest'' to the true model. @shibata1980asymptotically defined model selection to be *asymptotically efficiency* if this method chooses the candidate model with the smallest prediction error (in some sense) as the sample size goes to infinity. Under some typical conditions, the AIC as well as Mallow's $C_p$ are both asymptotically efficiency but the BIC and HQ are not. The second philosophy of model selection relies on *consistency* and is based on the idea that the true model is of **finite** dimension and that it is included in the set of candidate models. Under this asumption the goal of model selection is to correctly select this true model among the candidates models. Moreover, a model selection criterion is said to be *model selection consistent* if it identifies the correct model with porbability one as the sample size goes to infinity. The BIC and HQ are both *model selection consistent* but this is not the case for the AIC and $C_p$. The existance of the true model among the candidate model is often considered rather strong by statisticans. In practice, the efficiency approach tends to be more popular but there is little agreement and the choice between these two philosophies remains highly subjective. To make matters more confusing, both consistency and efficieny are asymptotic properties, which may not be "observable" in small samples. In this section, we will consider various simulations to better understand the small sample properties of AIC, BIC and HQ.
