...
# Basic Elements of Time Series {#introtimeseries} 
...
```{r MA1theoACF, cache = TRUE, echo = FALSE, fig.cap = "Comparison of theoretical ACF of MA(1) with different parameter values", fig.align='center', fig.height = 8, fig.width = 10, cache = TRUE, eval = FALSE}
par(mfrow=c(2,2))
plot(theo_acf(ar = NULL, ma = 0.9), 
     main = expression(paste("Theoretical ACF plot of MA(1) with ", theta, " = 0.9")))
plot(theo_acf(ar = NULL, ma = 0.5),
     main = expression(paste("Theoretical ACF plot of MA(1) with ", theta, " = 0.5")))
plot(theo_acf(ar = NULL, ma = -0.9)), 
     main = expression(paste("Theoretical ACF plot of MA(1) with ", theta, " = -0.9")))
plot(theo_acf(ar = NULL, ma = 0.1), 
...
```{example, name="Invertibility of MA(1) models", label = "invertma1"}
Let us consider an MA(1) model
...
```{definition, name="Invertible MA models", label = "invertMA"}
If an MA(q) model can be written as follows

\[\theta^{-1}(B) X_t = W_t,\]

where $\theta(B)$ is the moving average operator then the MA(q) model is invertible.
```
...
### Selecting and Forecasting MA($q$) Models
...
```{r, eval = FALSE, fig.asp = 0.73, fig.width = 7, fig.align='center', fig.cap="Values of the three model selection criteria for all candidate models included in an MA(16) model for the yearly Lynx trappings."}
...
select_lynx = suppressWarnings(simts::select(MA(16), lynx_gts)) 
```
...
Therefore, based on the AIC, we would nevertheless choose to model this time series using an AR(11) model. Supposing however that we had chosen the MA(10) model to describe and predict our time series: how would we deliver forecasts based on this model?

```{example, name="Forecasting MA models", label = "forecastMA"}
Let us assume we want to deliver a one-step ahead forecast based on an MA($q$) model. This is given by the conditional expectation

\[X_{t+1}^t = \mathbb{E}\left[X_{t+1} | X_t, ..., X_1, W_t ..., W_1\right] = \theta_1 W_t + ... + \theta_q W_{t-q+1}.\]

However, we do not observe the innovation process $(W_t)$ directly and therefore forecasting MA($q$) models is more complicated than forecasting AR($p$) models. For this reason there are different techniques such as the innovation algorithm or the Kalman filter that allow to deliver these forecasts. These techniques however are out of the scope of this book.
```
...
```{r, echo = FALSE}
library(simts)
suppressPackageStartupMessages(library(tsdl))
```
...
An ARMA($p$,$q$) can be written in concise form as:
		\[\phi \left( B \right) X_t = \theta \left( B \right)W_t, \]
...
Table: Features of the ACF and PACF plots for AR($p$), MA($q$) and ARMA($p$,$q$) models.
...
```{r, fig.asp = 0.6, fig.width = 7, fig.align='center', fig.cap="Annual Prices of Copper from 1880-1997 with long-term trend removed."}
...
```{r, echo = FALSE, cache = TRUE, fig.asp = 1.45, fig.width = 8, fig.align='center', fig.cap="Values of the three model selection criteria for all possible candidate models included in an ARMA(4,5) model."}
suppressWarnings(simts::select(ARMA(4,5), Xt))
```

```{r, eval = FALSE, fig.asp = 1.45, fig.width = 8, fig.align='center', fig.cap="Values of the three model selection criteria for all possible candidate models included in an ARMA(4,5) model."}
select(ARMA(4,5), Xt)
```
...
$$\delta^d X_t = (1 - B)^d X_t.$$
We have already seen some examples where a first-order difference ($d=1$) of a non-stationary time series can make the time series stationary. Indeed, a time series with a linear drift and/or a random walk can be made stationary by taking a first difference. 
