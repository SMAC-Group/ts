...
## Inference for AR(p) Models `r emo::ji("warning")`
...
## Model Selection `r emo::ji("warning")`
...
```{r simuStrongWeakFake, eval = FALSE}
B = 500
...
for(i in 1:B){
  ...
  tmp_aic = simts::select(AR(9), Xt1, criterion = "aic", plot = FALSE)
  tmp_bic = simts::select(AR(9), Xt1, criterion = "bic", plot = FALSE)
  tmp_hq = simts::select(AR(9), Xt1, criterion = "hq", plot = FALSE)
  ...
  tmp_aic = simts::select(AR(9), Xt2, criterion = "aic", plot = FALSE)
  tmp_bic = simts::select(AR(9), Xt2, criterion = "bic", plot = FALSE)
  tmp_hq = simts::select(AR(9), Xt2, criterion = "hq", plot = FALSE)
...
```{r simuStrongWeak, eval = TRUE, cache = TRUE, echo = FALSE}
B = 500
result1 = result2 = data.frame(aic = rep(NA, B), 
                               bic = rep(NA, B), 
                               hq = rep(NA, B))
...
```{r, fig.align='center', fig.height=4.5, fig.width=9, fig.cap="TO DO", echo = FALSE}
# plot the results
par(mfrow=c(1,2))
mycolor = hcl(h = seq(15, 375, length = 4), l = 65, c = 100, alpha = 1)
...
lynx_select = simts::select(AR(15), lynx_gts, include.mean = TRUE)
```
