...
$$C = E\left[ {{E_0}\left[ {\left\| {{X_0} - \hat X} \right\|_2^2} \right]} \right],$$
where $\|\cdot\|_2^2$ represents the L2-norm, $\hat{X}$ represents the predictions from a given (estimated) model, $X_0$ represents future unobserved values of the time series $(X_t)$ and $E_0$ represents the out-of-sample expectation (i.e. the expectation of my future values $X_0$). In general terms, this criterion is measuring the expected difference between my predicted values and the expected future values of my time series. Therefore, a "good" model would be a model that minimizes this criterion thereby minimising the expected "prediction error" of the model.

Assuming that this is a good criterion to evaluate the "goodness" of a model, most of the quantites defining this criterion are not directly available and therefore we need estimators for this criterion. The most popular criteria that aim at estimating this quantity are defined as follows:

\begin{align*}
  AIC &=  -2 \text{logL}(\hat{theta}) + 2p\\
  BIC &=  -2 \text{logL}(\hat{theta}) + \log(T) p\\
  HQ  &=  -2 \text{logL}(\hat{theta}) + 2\log(\log(T)) p
\end{align*}
