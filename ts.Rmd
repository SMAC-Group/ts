...
```{r, fig.asp = 0.75, fig.width = 6, fig.align='center', fig.cap="TO DO"}
...
```{r, fig.asp = 0.6, fig.width = 7, fig.align='center', fig.cap="TO DO"}
...
plot(NA, xlim = c(0.5,1.04), ylim = range(yy.gmwm),
...
plot(NA, xlim = c(0.35,1.04), ylim = range(yy.gmwm),
     xlab = expression(phi), ylab = "Density", main = "GMWM")
grid()
abline(v = 1, lwd = 2, col = "grey60")
abline(v = 0.96, lwd = 2, lty = 2, col = "grey60")
plot(hist.phi.GMWM, xlim = c(0.53,1.2), ylim = range(yy.gmwm),
...
```{r mbbdata, fig.asp = 0.45, fig.width = 8, fig.align='center', caption = "TO DO", cache = FALSE}
set.seed(1)         # Set seed for reproducibilty
n = 500             # Sample size
xt_m1 = gen_gts(n, AR1(phi = 0.5, sigma2 = 1))   # Gaussian noise only
xt_m2 = gts(arima.sim(n = n, list(ar = 0.5, ma = 0), # Student-t noise
rand.gen = function(n, ...) rt(n, df = 4)))
```


```{r mbbdatavis, echo = F, cache = FALSE, fig.cap="AR(1) processes generated under different noise conditions.", fig.asp = 0.5, fig.width = 8, fig.align='center', caption = "TO DO", dependson="mbbdata"}
par(mfrow = c(1,2))
plot(xt_m1, main = "Model 1")
plot(xt_m2, main = "Model 2")
```

From Figure \@ref(fig:mbbdatavis), the two time series appear to be considerably different as a result of the noise process being altered even though the $\phi$ coefficient is held constant. Among others, the difference is also related to the variance of the $t$ distribution defined as $\frac{\nu}{\nu-2}$ for $\nu > 2$ (and $\infty$ for $\nu \le 2$). Therefore, the innovation variance of the i.i.d white noise process is given by $\sigma^2_{\mathcal{M}_1} = 1$ while the noise generated from the $t$ distribution has innovation variance $\sigma^2_{\mathcal{M}_2} = 2$. To compare the performance of the bootstrap procedures, the function below performs parametric bootstrap for the MLE of an AR(1) process assuming the innovations are Gaussian.
...
Now that we have two functions implementing the block and parametric bootstraps, we can apply these procedures to obtain confidence intervals for the AR(1) parameters for the two time series generated respectively by model $\mathcal{M}_1$ and $\mathcal{M}_2$.

```{r blockbootsim, cache = FALSE}
B = 1000
...
plot(NA, xlim = c(0.33,0.62), ylim = c(0,18),
xlab = expression(phi), ylab = "Density", main = "Model 1")
grid()
abline(v = 1, lwd = 2, col = "grey60")
abline(v = 0.95, lwd = 2, lty = 2, col = "grey60")
plot(hist.phi.ML, xlim = c(0.33,0.62), ylim = c(0,18),
add = T, col = colors[3], border = "grey60")

plot(hist.phi.ML.bboot, xlim = c(0.33,0.62), ylim = c(0,18),
...
The results from the parametric and nonparametric block bootstrapping techniques are displayed in Figure \@ref(fig:blockbootmodels). In both instances, we can  see that the block bootstrap has a narrower distribution. However, there is a considerable bias that leads the distribution to be off-centered. The origins of this bias comes from the independence associated between blocks as they are placed into $(X^*_t)$. 
...
All these criteria in some way represent (under different forms) the general criterion $C$ defined earlier. Assuming that $C$ is a good criterion to evaluate the "goodness" of a model, most of the quantites defining th criterion are not directly available and therefore we need estimators for this criterion. Denoting $\text{logL}(\hat{\boldsymbol{\theta}})$ as being the log-likelihood function evaluated at the estimated parameters and $k$ as the number of parameters in the model, the three criteria higlighted in the previous paragraph aim at estimating this quantity as follows:
...
```{r, fig.height= 4.5, fig.width= 5, fig.align='center', fig.cap="TO DO."}
set.seed(1)
...
```{r, fig.height= 5.5, fig.width= 7.5, fig.align='center', fig.cap="TO DO."}
...
In this simulation study, we consider the following two models:
...
We consider sample size $N = 100$ and bootstrap replications $B = 500$. 
```

```{r simuStrongWeak, eval = TRUE, cache = TRUE}
B = 500
result1 = result2 = data.frame(aic = rep(NA, B), 
                               bic = rep(NA, B), 
                               hq = rep(NA, B))
...
for(i in 1:B){
  ...
  # generate data
  Xt1 = gen_gts(n = 100, AR(phi = c(0.6, 0.3), sigma2 = 1))
  Xt2 = gen_gts(n = 100, AR(phi = c(0.5, 0.25, 0.125, 0.0625), sigma2 = 1))
  
  # model selection for Xt1
  tmp_aic = select(AR(9), Xt1, criterion = "aic", plot = FALSE)
  tmp_bic = select(AR(9), Xt1, criterion = "bic", plot = FALSE)
  tmp_hq = select(AR(9), Xt1, criterion = "hq", plot = FALSE)
  
  result1$aic[i] = length(tmp_aic$model$phi)
  result1$bic[i] = length(tmp_bic$model$phi)
  result1$hq[i] = length(tmp_hq$model$phi)
  
  # model selection for Xt2
  tmp_aic = select(AR(9), Xt2, criterion = "aic", plot = FALSE)
  tmp_bic = select(AR(9), Xt2, criterion = "bic", plot = FALSE)
  tmp_hq = select(AR(9), Xt2, criterion = "hq", plot = FALSE)
  
  result2$aic[i] = length(tmp_aic$model$phi)
  result2$bic[i] = length(tmp_bic$model$phi)
  result2$hq[i] = length(tmp_hq$model$phi)
}
...
aic1 = bic1 = hq1 = aic2 = bic2 = hq2 = rep(0, 9)
...
barplot(result1, 
        col = mycolor[1:3],
        beside = T, ylim = c(0, max(result1)+1),
        main = "Strong Signal", 
        xlab = "autoregressive order (p)", ylab = "counts")
legend("topright",
       legend = c("AIC", "BIC", "HQ"),
       bty = "n",
       fill = mycolor[1:3])
...
barplot(result2, 
        col = mycolor[1:3],
        beside = T, ylim = c(0, max(result2)+1),
        main = "Weak Signal", 
        xlab = "autoregressive order (p)", ylab = "counts")
legend("topright",
       legend = c("AIC", "BIC", "HQ"),
...
Having studied th performance of these three criteria through a simultation study, let us now consider an example we considered when explaining the usefulness of the PACF to determine the order of an AR(p) model. For this reason, let us again check the PACF of the Lynx trapping data set represented below.

```{r}
plot(auto_corr(lynx_gts, pacf = TRUE))
```

As we saw earlier, the PACF appears to be significant at lag $h = 8$ therefore leading to the conclusion that an AR(8) model would be good to describe/forecast the observed time series. However, let us check if we reach the same conclusion using the three model selection criteria described above. For this reason, let us use an AR(15) as the maximum order for the candidate AR(p) models.

```{r}
lynx_select = select(AR(15), lynx_gts, include.mean = TRUE)
```

In this case, we see that the AIC and HQ select an AR(8) model in accordance with the selection procedure based on the ACF. However the BIC selects a much smaller model (AR(3)). This can be due to different reasons but the BIC requires the presence of the true model among the candidate models and this is possibly not the case for this example. Hence, for this case the AIC or HQ are probably better choices.

To summarize, these techniques are all aiming at estimating the out-of-sample prediction error of a given model. However, they strongly rely on the chosen likelihood function and, if this does not adequately represent the model that generated the observed time series, then they could deliver misleading conclusions. For this reason, one could consider another approach to model selection based on model forecasting accuracy. Indeed, one could select a model based on the following procedure:
...
2 Estimate the model you wish to evaluate on the training set and forecast the next observation (i.e. $X_{n+1}^n$). Compute the difference between your forecast and the actual value $X_{n+1}$.
3 Add the observation $X_{n+1}$ to the training set and let $n = n + 1$. Go to Step 2 until $n = T$.
4 Compute a suitable “score” to asses the quality of your model based on the
empirical “prediction errors” vector.
