...
where $\theta(B)$ is the moving average operator such that 

\[\theta^{-1}(B) = \sum_{j = 0}^{\infty} (-\theta)^j B^j,\] 

then the MA(q) model is invertible.
```
...
\[X_{t+1}^t = E\left[X_{t+1} | X_t, ..., X_1, W_t ..., W_1\right] = \theta_1 W_t + ... + \theta_q W_{t-q+1}\].
